 ...
 * @memberof ticker
 */
export type TickerCallback<T> = (this: T, ticker: ITicker<T>) => any;
...
export interface ITicker<T> {
  ...
  speed: number;
  started: boolean;
  add: (fn: TickerCallback<T>, context?: T, priority?: number) => ITicker<T>;
  addOnce: (
    fn: TickerCallback<T>,
    context?: T,
    priority?: number,
  ) => ITicker<T>;
  remove: (fn: TickerCallback<T>, context?: T) => ITicker<T>;
  count: number;
  start: () => void;
  stop: () => void;
  destroy: () => void;
...
export class Ticker {
  ...
  constructor() {
    //@ts-expect-error this is as it exists in @pixi/ticker
    ...
    this._tick = (time: number): void => {
      ...
      if (this.started) {
        ...
        if (this.started && this._requestId === null && this._head.next) {
          this._requestId = setTimeout(this._tick, 0);
        }
      }
    };
  }
   ...
   * @returns This instance of a ticker
   */
  public remove<T>(fn: TickerCallback<T>, context?: T): this {

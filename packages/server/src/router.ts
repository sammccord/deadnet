import type { IMessage } from '@deadnet/bebop/lib/bebop';
import { Message } from '@deadnet/bebop/lib/bebop';
import { createEventIterator } from '@deadnet/bebop/lib/createEventIterator';
import {
...
> {
  ...
  private readonly clientStreams: Map<
    ...
    Promise<any>
  > = new Map();
  private readonly topicStreams: Map<
    string,
    AsyncGenerator<BebopRecord, void, unknown>
  > = new Map();
  ...
  private async invokeClientStreamMethod(
    ...
    if (isStreaming) {
      ...
      this.events.dispatchEvent(
        new CustomEvent(messageId, { detail: request }),
      );
  ...
  private async invokeDuplexStreamMethod(
    ...
    if (topic) {
      ...
      if (isStreaming) {
        const invocation = this.topicStreams.get(topic)!;
    ...
    const generator = () => {
      return createEventIterator<BebopRecord>(({ emit, cancel }) => {
        ...
        if (topic) {
          ...
          this.events.dispatchEvent(
            new CustomEvent(topic, { detail: request }),
          );
        }
        this.events.addEventListener(
          messageId,

...
export class PixelIt {
  ...
  constructor(config: {
    ...
    //target for canvas
    this.drawto = createCanvas(this.maxHeight, this.maxWidth)
   ...
   */
  setFromImgSource(src: string | Buffer) {
   ...
   */
  setPalette(arr: PixelIt['palette']) {
    this.palette = arr
    return this
  }
  ...
  colorSim(rgbColor: [number, number, number], compareColor: [number, number, number]) {
    let i
    let max
    let d = 0
   ...
   */
  similarColor(actualColor: [number, number, number]) {
  ...
  */
    ...
    if (this.drawfrom.naturalWidth > 300) {
      finalWidth +=
        this.drawfrom.naturalWidth > this.drawfrom.naturalHeight
          ? parseInt(
            String(this.drawfrom.naturalWidth / (this.drawfrom.naturalWidth * this.scale))
          ) / 1.5
          : parseInt(String(this.drawfrom.naturalWidth / (this.drawfrom.naturalWidth * this.scale)))
    }
    ...
    if (this.drawfrom.naturalHeight > 300) {
      finalHeight +=
        this.drawfrom.naturalHeight > this.drawfrom.naturalWidth
          ? parseInt(
            String(this.drawfrom.naturalHeight / (this.drawfrom.naturalHeight * this.scale))
          ) / 1.5
          : parseInt(
            String(this.drawfrom.naturalHeight / (this.drawfrom.naturalHeight * this.scale))
          )
  ...
  resizeImage() {
    ...
    const canvasCopy = createCanvas(this.maxWidth, this.maxHeight)
